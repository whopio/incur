...
export async function generateCommands(
  ...
  for (const { method, operation: op, path } of operations) {
    ...
    for (const [key, schema] of Object.entries(bodyProps)) {
      ...
      if (typeof schema.description === 'string') zodType = zodType.describe(schema.description)
 ...
 */
function responseSchema(op: Operation): Schema.JsonSchema | undefined {
  const responses = op.responses ?? {}
  const keys = Object.keys(responses)
  const key =
    keys.find((k) => k === '200' || k === '201') ??
    keys.find((k) => /^2(\d\d|xx)$/i.test(k)) ??
    keys.find((k) => k === 'default')
  if (!key) return undefined
  const response = responses[key] as
    | { content?: Record<string, { schema?: Schema.JsonSchema | undefined }> | undefined }
    | undefined
  const schema = response?.content?.['application/json']?.schema
  if (!schema || typeof schema !== 'object' || !acyclic(schema)) return undefined
  return schema
}

/** Whether an object graph can be serialized: no object reachable from itself. */
function acyclic(root: unknown): boolean {
