...
export const name = typeof __INCUR_BINARY_NAME__ === 'string' && __INCUR_BINARY_NAME__
    ? __INCUR_BINARY_NAME__
    : undefined;
/** Target embedded by `incur build`, or `undefined` outside a compiled artifact. */
export const target = typeof __INCUR_BINARY_TARGET__ === 'string' && __INCUR_BINARY_TARGET__
    ? __INCUR_BINARY_TARGET__
    : undefined;
/** Version embedded by `incur build`, or `undefined` outside a compiled artifact. */
export const version = typeof __INCUR_BINARY_VERSION__ === 'string' && __INCUR_BINARY_VERSION__
 ...
 */
export async function handleArgv(argv = process.argv.slice(2)) {
    if (argv[0] !== applyFlag) {
    ...
    if (process.platform !== 'win32')
        throw new Error(`${applyFlag} is only supported on Windows.`);
...
function releaseAssetName(binaryName, binaryTarget) {
...
async function fetchReleases(repository) {
    const releases = [];
    let url = `${githubApi}/repos/${repository}/releases?per_page=100`;
    while (url) {
        ...
        if (!response.ok)
            throw new Error(`GitHub release request failed with ${response.status} ${response.statusText}.`);
...
function selectRelease(releases, current, assetName) {
    let selected;
    for (const release of releases) {
        ...
        if (!releaseVersion || !isNewerVersion(releaseVersion, current))
            continue;
...
function hasNewerStableRelease(releases, current) {
...
function stableVersion(tag) {
    const match = tag.match(/^(?:[vV]?|(?:@[A-Za-z0-9][A-Za-z0-9._~-]*\/)?[A-Za-z0-9][A-Za-z0-9._~-]*@)(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/);
    if (!match)
        return undefined;
    return `${match[1]}.${match[2]}.${match[3]}`;
}
/** Returns an asset matcher that validates the fields used during installation. */
function isAssetNamed(assetName) {
    return (value) => {
        if (!value || typeof value !== 'object')
            return false;
        const asset = value;
...
async function installRelease(release) {
    ...
    if (!digest)
        throw new Error(`Release asset '${release.asset.name}' has no SHA-256 digest.`);
    ...
    try {
        ...
        if (process.platform === 'win32')
            await startWindowsHandoff(paths);
        else
            await fs.promises.rename(paths.staged, process.execPath);
    }
    catch (error) {
...
async function decompress(compressed, assetName) {
    try {
...
async function executableMode(file) {
    try {
...
async function validateCandidate(file, expected) {
    const output = await execFile(file, ['--version']);
    if (output.trim() !== expected)
        throw new Error(`Downloaded binary reported version '${output.trim() || 'unknown'}', expected '${expected}'.`);
}
/** Runs an executable and captures stdout while preserving useful failure output. */
function execFile(command, args) {
    return new Promise((resolve, reject) => {
        childProcess.execFile(command, args, { encoding: 'utf8', timeout: requestTimeout }, (error, stdout, stderr) => {
            if (!error)
...
function updatePaths(executable) {
...
async function startWindowsHandoff(state) {
...
function pathsFromHelper(helper, parent) {
    ...
    if (!match?.[1] || !match[2])
        throw new Error('Invalid binary update handoff path.');
...
async function cleanupArtifacts(executable) {
    ...
    try {
        ...
        await Promise.all(entries
            ...
            /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.exe$/i.test(entry.slice(prefix.length)))
