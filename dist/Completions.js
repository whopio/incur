 ...
 */
export function complete(commands, rootCommand, argv, index, globals) {
    ...
    if (index > 0) {
        ...
        if (prev.startsWith('-')) {
            ...
            // Try global options
            if (globals) {
                const globalEntry = { options: globals.schema, alias: globals.alias };
                ...
                if (name) {
                    ...
                    if (!isBooleanOption(name, globals.schema))
                        return candidates;
                }
            }
        }
